import React from "react";

interface IconHeatingProps {
  color?: string;
  size?: number;
}

const IconHeating: React.FC<IconHeatingProps> = ({
  color = "#1A1A1A",
  size = 24,
}) => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width={size}
      height={size}
      viewBox="0 0 24 24"
      fill="none"
    >
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12 1.5C12.4142 1.5 12.75 1.83579 12.75 2.25V4.5C12.75 4.91421 12.4142 5.25 12 5.25C11.5858 5.25 11.25 4.91421 11.25 4.5V2.25C11.25 1.83579 11.5858 1.5 12 1.5Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12 18.75C12.4142 18.75 12.75 19.0858 12.75 19.5V21.75C12.75 22.1642 12.4142 22.5 12 22.5C11.5858 22.5 11.25 22.1642 11.25 21.75V19.5C11.25 19.0858 11.5858 18.75 12 18.75Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M19.4247 4.57529C19.7176 4.86819 19.7176 5.34306 19.4247 5.63595L17.8338 7.22689C17.5409 7.51979 17.066 7.51979 16.7731 7.22689C16.4802 6.934 16.4802 6.45913 16.7731 6.16623L18.364 4.57529C18.6569 4.2824 19.1318 4.2824 19.4247 4.57529Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M7.22689 16.7731C7.51979 17.066 7.51979 17.5409 7.22689 17.8338L5.63595 19.4247C5.34306 19.7176 4.86819 19.7176 4.57529 19.4247C4.2824 19.1318 4.2824 18.6569 4.57529 18.364L6.16623 16.7731C6.45913 16.4802 6.934 16.4802 7.22689 16.7731Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M18.75 12C18.75 11.5858 19.0858 11.25 19.5 11.25H21.75C22.1642 11.25 22.5 11.5858 22.5 12C22.5 12.4142 22.1642 12.75 21.75 12.75H19.5C19.0858 12.75 18.75 12.4142 18.75 12Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M1.5 12C1.5 11.5858 1.83579 11.25 2.25 11.25H4.5C4.91421 11.25 5.25 11.5858 5.25 12C5.25 12.4142 4.91421 12.75 4.5 12.75H2.25C1.83579 12.75 1.5 12.4142 1.5 12Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M16.7731 16.7731C17.066 16.4802 17.5409 16.4802 17.8338 16.7731L19.4247 18.364C19.7176 18.6569 19.7176 19.1318 19.4247 19.4247C19.1318 19.7176 18.6569 19.7176 18.364 19.4247L16.7731 17.8338C16.4802 17.5409 16.4802 17.066 16.7731 16.7731Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M4.57529 4.57529C4.86819 4.2824 5.34306 4.2824 5.63595 4.57529L7.22689 6.16623C7.51979 6.45913 7.51979 6.934 7.22689 7.22689C6.934 7.51979 6.45913 7.51979 6.16623 7.22689L4.57529 5.63595C4.2824 5.34306 4.2824 4.86819 4.57529 4.57529Z"
        fill={color}
      />
      <path
        fillRule="evenodd"
        clipRule="evenodd"
        d="M12 9C10.3431 9 9 10.3431 9 12C9 13.6569 10.3431 15 12 15C13.6569 15 15 13.6569 15 12C15 10.3431 13.6569 9 12 9ZM7.5 12C7.5 9.51472 9.51472 7.5 12 7.5C14.4853 7.5 16.5 9.51472 16.5 12C16.5 14.4853 14.4853 16.5 12 16.5C9.51472 16.5 7.5 14.4853 7.5 12Z"
        fill={color}
      />
    </svg>
  );
};

export default IconHeating;
